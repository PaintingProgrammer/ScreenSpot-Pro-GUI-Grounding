...
def main():
    ...
    outputs = llm.generate(
        ...
        sampling_params=SamplingParams(max_tokens=50, top_k=1, stop=["<｜end▁of▁sentence｜>"]),
    )
    ...
    for o in outputs:
        ...
        print(response)
        coords = ast.literal_eval(response.replace("<｜end▁of▁sentence｜>", "").replace("```", "").replace(" ", "").strip())
